...
# Vietnamese graph2phoneme
def g2p(txt):
    return list(txt)

# phone_dict = list(" !\"$%'()*+,-.0123456789:?\_abcdefghijklmnopqrstuvwxyzàáâãèéêìíðòóôõùúýăđĩũơự̀́̃ạảấầẩẫậắằẳẵặẹẻẽếềểễệỉịọỏốồổỗộớờởỡợụủứừửữựỳỵỷỹ’“”…")

phone_dict = list(" abcdefghijklmnopqrstuvwxyzàáâãèéêìíòóôõùúýăđĩũơạảấầẩẫậắằẳẵặẹẻẽếềểễệỉịọỏốồổỗộớờởỡợụủưứừửữựỳỵỷỹ")
...
def load(path, sr=22050, mono=False, offset=0., duration=None):
